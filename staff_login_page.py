...
class StaffLoginPage:
    ...
    def __init__(self, window):
        ...
        # ============ Sign In Image =============================================
        # ========================================================================
        self.sign_in_image_staff = Image.open('images\manager.jpg')
        self.resized=self.sign_in_image_staff.resize((150,150))
        photo = ImageTk.PhotoImage(self.resized)
        self.sign_in_image_staff_label = Label(self.lgn_frame, image=photo, bg='#FFFFFF')
        self.sign_in_image_staff_label.image = photo
        self.sign_in_image_staff_label.place(x=800, y=50)
    ...
    def signIn(self):
        StaffLoginPage.entered_username =self.username_entry.get()
        StaffLoginPage.entered_pass = self.password_entry.get()

        find_user= ("""select staff_id, password
        from staff
        where staff_id ==? and password==?""")

        sql_test.sqlBase.cursor.execute(find_user,[StaffLoginPage.entered_username,StaffLoginPage.entered_pass])
